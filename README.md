...
|`ca_file`   | Specify an optional file to use for certificate authorities | This option may be a file containing a list of certificate authorities, or it may be an empty string, in which case the system defailt certificate authorities are used.|
|`debug`     | Enables various behaviours related to debugging. This option is insecure, as it uses SSL without verifiying certificates. | For production, always use `false`. For testing, you may consider setting this to `true`, which may allow testing SSL based work without a proper certificate.|
